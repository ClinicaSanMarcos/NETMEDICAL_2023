...
{
    public partial class frmInformacionComplementariaEmergencia : Form
    {
        ...
        public frmInformacionComplementariaEmergencia(string _HospitalizacionId)
        {
        ...
        private void frmInformacionComplementariaEmergencia_Load(object sender, EventArgs e)
        {
            ...
            #endregion
            var cadena1 = @"select ISNULL(i_Hospitalizado, -1) as Hospitalizado,
                            ISNULL(i_Procedimiento, -1) as Procedimiento,
            ...
            while (lector.Read())
            {
                Hospitalizado = int.Parse(lector.GetValue(0).ToString());
                Procedimiento = int.Parse(lector.GetValue(1).ToString());

                TipoIngreso = int.Parse(lector.GetValue(2).ToString());
                ServicioId = lector.GetValue(3).ToString();
            ...
            if (Hospitalizado == 1)
            {
                rbHospitalizado.Checked = true;
                rbNoHospitalizado.Checked = false;

            }
            else if (Hospitalizado == 0)
            {
                rbHospitalizado.Checked = false;
                rbNoHospitalizado.Checked = true;
            }
            else
            {
                rbHospitalizado.Checked = false;
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Seleccione si se realizo un Procedimiento.", "Advertencia!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            int hospitalizacion_ = -1;

            if (rbHospitalizado.Checked == true)
            {
                hospitalizacion_ = 1;
            }
            else if (rbNoHospitalizado.Checked == true)
            {
                hospitalizacion_ = 0;
            }
            else
            {
                MessageBox.Show("Seleccione si se hospitalizo o no.", "Advertencia!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            var cadena1 = "update hospitalizacion set i_Hospitalizado = " + hospitalizacion_ + ", i_Procedimiento = " + procedimiento_ + " where v_HopitalizacionId = '" + HospitalizacionId + "'";
            SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
            SqlDataReader lector = comando.ExecuteReader();
            lector.Close();
