...
namespace Sigesoft.Node.WinClient.BLL
{
    ...
    {
        public string AddAdditionalExam(List<AdditionalExamCustom> listAdditionalExam, List<string> ClientSession)
        {
            try
            {
                ...
                if (listAdditionalExam[0].v_IdGrupoAdicional != string.Empty && listAdditionalExam[0].v_IdGrupoAdicional != "")
                {
                    NewIdOrden = listAdditionalExam[0].v_IdGrupoAdicional;
                }
                else
                {
                    NewIdOrden = Common.Utils.GetNewId(NodeId, Utils.GetNextSecuentialId(NodeId, 50), "OM");
                }
                ...
                foreach (var exam in listAdditionalExam)
                {
                    ...
                    objAdditionalExam.v_IdGrupoAdicional = NewIdOrden;
        ...
        public List<AdditionalExamCustom> GetAdditionalExamByServiceId_Por_Orden(string serviceId, int userId, string ordenId)
        {
            SigesoftEntitiesModel dbcontext = new SigesoftEntitiesModel();

            var list = (from ade in dbcontext.additionalexam
                where ade.v_ServiceId == serviceId && ade.i_IsDeleted == 0 && ade.i_InsertUserId == userId && ade.v_IdGrupoAdicional == ordenId
        ...
        public List<AdditionalExamUpdate> GetAdditionalExamForUpdateByServiceId(string serviceId, int userId)
        {
            ...
            var list = (from ade in dbcontext.additionalexam
                ...
                select new AdditionalExamUpdate
                {
                    ...
                    v_IdGrupoAdicional = ade.v_IdGrupoAdicional
                }).ToList();
