...
namespace Sigesoft.Node.WinClient.BLL
{
   public class CalendarBL
   {
       ...
       public List<ListColaEspera> GetCallPacientesColaEspera(ref OperationResult pobjOperationResult)
       {
           //mon.IsActive = true;
           try
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
               var query = (from A in dbContext.listllamadadeespera(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day)
                            select new ListColaEspera
                            {
                                PACIENTE = A.PACIENTE,
                                HORARIO = A.HORARIO,
                                Medico = A.Medico,
                            ...
                            }).OrderBy(p=>p.d_ServiceDate).ToList();
               var medico = query.GroupBy(g => g.Medico).Select(s => s.First()).ToList();

               List<ListColaEspera> listaGlobal = new List<ListColaEspera>();

               foreach (var item in medico)
               {
                   int contador = 1;

                   var list = query.FindAll(p => p.Medico == item.Medico).ToList();

                   foreach (var item2 in list)
                   {
                       ListColaEspera objListColaEspera = new ListColaEspera();
                       objListColaEspera.N = contador;
                       objListColaEspera.PACIENTE = item2.PACIENTE;
                       objListColaEspera.HORARIO = item2.HORARIO;
                       objListColaEspera.Medico = item2.Medico;
                       objListColaEspera.TURNO = item2.TURNO;
                       objListColaEspera.d_ServiceDate = item2.d_ServiceDate;
                       listaGlobal.Add(objListColaEspera);
                       contador++;
                   }
               }
               pobjOperationResult.Success = 1;
